package com.gbi.hnapay.util;

import cn.hutool.core.util.StrUtil;
import com.gbi.hnapay.config.HnaPayConfig;
import com.gbi.hnapay.exception.HnaPayCryptoException;
...
public class HnaPayRsaUtil {
	...
	private static String signature(byte[] data, String privateKey, String signatureKey) {
		...
		try {
			...
			str = Base64Utils.encodeToString(signature.sign());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return str;
	...
	private static boolean verifyData(byte[] data, String publicKey, String sign, String signatureKey) {
		...
		try {
			...
			signature.update(data);
			flag = signature.verify(Base64Utils.decodeFromString(sign));
		} catch (Exception e) {
			return false;
		}
		return flag;
	 ...
	 */
	@Nullable
	public static String encryptSliceToBase64(String base64PublicKey, @Nullable String data) {
